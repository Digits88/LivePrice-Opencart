...
class ModelModuleLivePrice extends Model {
  ...
  public function getPriceStartingFrom($product_info, $price, $special, $tax) {
    ...
    if ( $this->installed() && ($this->customer->isLogged() || !$this->config->get('config_customer_price')) && $this->versionPRO() ) {
      ...
      if ( !empty($lp_settings['starting_from']) ) {
        ...
        if ( $starting_prices ) {
          if ( ($lp_settings['show_from'] == 1 && $starting_prices['minimal'] )  
        ...
        } elseif ( !empty($lp_settings['show_from']) && $lp_settings['show_from'] == 2 ) { 
  ...
  function getRelatedOptionsType() {
    if ( is_null($this->ro_type) ) {
      ...
      if ( $this->installedRelatedOptions() ) {
        ...
        if ( method_exists( 'ModelModuleRelatedOptions', 'get_related_options_set_by_poids' ) ) { 
          $this->ro_type = 'ro';
        } elseif ( method_exists( 'ModelModuleRelatedOptions', 'get_related_options_sets_by_poids' ) ) { 
          $this->ro_type = 'ropro';
        }
      }
    }
  ...
  function getProductDefaultPrice($product_id, $inclide_starting_from = false) {  
    ...
    if ( $this->installed() && $this->versionPRO() ) {
      ...
      if ( !empty($lp_settings['default_price']) ) { 
        ...
        if ( $this->installedImprovedOptions() ) {
          ...
          if ( !empty($io_settings['auto_selection']) ) {
            ...
            foreach ($query->rows as $row) {
              ...
              if ( $io_settings['auto_selection'] == 1 ) { 
              ...
              } elseif ( $io_settings['auto_selection'] == 2 ) { 
              ...
              } elseif ( $io_settings['auto_selection'] == 3 ) { 
            ...
            if ( $query->num_rows ) {
              ...
              foreach ( $query->rows as $row ) {
                if ( $row['price'] != $product_price ) {
                  $ro_minimal = $this->getROCombination( $query->rows[0]['relatedoptions_id'] ); 
                  break;
                }
              }
      ...
      foreach ($query->rows as $row) {
        ...
        if ( !isset($values_for_calc[$row['product_option_id']]) ) {
          ...
          if ( $inclide_starting_from && isset($ro_minimal[$row['product_option_id']]) ) { 
          ...
          } elseif ( $inclide_starting_from ) { 
            if ( !empty($lp_settings['starting_from']) && $lp_settings['starting_from'] == 1 && $row['required'] ) { 
              $values_for_calc[$row['product_option_id']] = $row['product_option_value_id'];
              
            } elseif ( !empty($lp_settings['starting_from']) && $lp_settings['starting_from'] == 2 ) { 
              $values_for_calc[$row['product_option_id']] = $row['product_option_value_id'];
            }
          }
  ...
  private function arrayKeysToInt($arr) {
	...
	public function getProductPriceByParamsArray($params) {
		...
		return $this->getProductPrice($product_id, $quantity, $options, $recurring_id, $prices, $product_data, $option_data, $multiplied_price, $use_cart_cache, $use_price_cache, $without_discounts, $ignore_cart);
  ...
  public function getProductPrice($p_product_id, $current_quantity=1, $options=array(), $recurring_id=0, $prices=array(), $product_data=array(), $option_data=array(), $multiplied_price=false, $use_cart_cache=false, $use_price_cache=false, $without_discounts=false, $ignore_cart=false) {
